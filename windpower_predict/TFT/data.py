...
def get_batches(data_, in_seq_len, out_seq_len, con_cols, disc_cols, target_cols, batch_size = 1, gpu = True, normalise = True, indexer = None, norm = None):
    ...
    if normalise:
        ...
        data[con_cols] = (data[con_cols] - norm[con_cols].stack().min()) / (norm[con_cols].stack().max() - norm[con_cols].stack().min())
    ...
    while True:
        ...
        yield (torch.as_tensor(in_seq_continuous).type(dtype).unsqueeze(-1),
                        torch.as_tensor(in_seq_discrete).type(dtype),
                        torch.as_tensor(out_seq).type(dtype),
                        torch.as_tensor(target_seq).type(dtype))
